...
//  GOTList
//
//  Created by Gabriel on 24/11/18.
...
class TableViewController: UITableViewController {
    ...
    func GetData(){
        ...
        do {
            let result = try context.fetch(Character.fetchRequest())
            Actors = result as! [Character] //convert
        }
        catch{
            print("Data Error")
        }
    ...
    func Alert(){
        let alert = UIAlertController(title: "Add Character",
                                      message: "Please add only characters in GoT",
